...
Container components SHALL use NgRx functional effects with `createEffect` and `{ functional: true }` for side effects. Effects SHALL be exported as arrays and registered in `app.config.ts`.
...
- **AND** effects SHALL be exported as arrays (e.g., `offersEffects = [...]`)
